...
def construct_model(config):
    from models.quognn import quognn
    ...
    stdam = np.load(config['adj_confuse_filename'])
    
    adj_mx = construct_adj_fusion(adj, stdam, 4)
    ...
    if first_layer_embedding_size:
        input_data = mx.sym.slice(data, begin=(None, 0, None, None), end=(None, 12, None, None))
        mutliple_data = mx.sym.slice(data, begin=(None, 12, None, None), end=(None, 13, None, None))
...
def generate_from_data(data, length, transformer):
    ...
    for line1, line2 in ((0, train_line),
        ...
        c = np.empty([x.shape[0], x.shape[2]])
        if line1 == 0:

...
def quognn(input_data, multiple_data, adj, label,
           ...
           prefix="", rho=1, predict_length=12, quotient_w=1):
    '''
    ...
    for i in range(predict_length):
        need_concat.append(
            output_layer(
                input_data, num_of_vertices, input_length, num_of_features,
                num_of_filters=128, predict_length=1
            )
        )
    input_data = mx.sym.concat(*need_concat, dim=1)
    multiple_data = mx.sym.reshape(multiple_data, (-1, num_of_vertices))
    multiple_data = mx.sym.reshape(multiple_data, (-1))
    multiple_data = mx.sym.diag(multiple_data)

    input_data = input_data + quotient_w

    input_data = mx.sym.transpose(input_data, (0, 2, 1))
    input_data = mx.sym.reshape(input_data, (-1, 12))
    input_data = mx.sym.transpose(input_data, (1, 0))
    input_data = mx.sym.dot(input_data, multiple_data)
    input_data = mx.sym.transpose(input_data, (1, 0))
    input_data = mx.sym.reshape(input_data, (-1, num_of_vertices, 12))
    input_data = mx.sym.transpose(input_data, (0, 2, 1))

    loss = huber_loss(input_data, label, rho=rho)
    return mx.sym.Group([loss, mx.sym.BlockGrad(input_data, name='pred')])
